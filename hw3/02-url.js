...
const server = http.createServer((req, res) => {
    ...
    let cssStyle = '"border:1px solid black"';
    let tableHTML = `<table style = ${cssStyle}>`;
    queryParameters.forEach((element, index) => {
        tableHTML += `<tr style=${cssStyle}><td style=${cssStyle}>${index}</td><td style=${cssStyle}>${element}</td>`;
    });
    tableHTML += '</table>';
    res.write(tableHTML);
    res.end();
